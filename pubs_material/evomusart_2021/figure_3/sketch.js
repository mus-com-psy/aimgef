...
function pointsetplot(x, y, px, py) {
  ...
  for (let i = 0; i < x.rngY / 5; i++) {
    ...
    pop()
    text(`${x.minY + 5 * i - 1}`, x.x - x.ppb - 60, x.y + x.ppp - x.ppp * 5 * (i) + 10);
    text(`${y.minY + 5 * i - 1}`, y.x - y.ppb - 60, y.y + y.ppp - y.ppp * 5 * (i) + 10);
  }
  for (let i = 0; i < x.rngX; i++) {
    ...
    if (rx === 1) {
      ...
      pop()
      text(`${x.minX + i}`, x.x + x.ppb * 4 * (i) - 25, x.y + x.ppp + 50);
    } else {
      push()
      strokeWeight(4);
      line(x.x + x.ppb * (4 * (i) + rx), x.y + x.ppp, x.x + x.ppb * (4 * (i) + rx), x.y + x.ppp + 10);
      pop()
      text(`${x.minX + rx + i}`, x.x + x.ppb * 4 * (i) - 15, x.y + x.ppp + 50);
    }
    if (ry === 1) {
      push()
      strokeWeight(4);
      ...
      pop()
      push()
      translate(y.x + y.ppb * 4 * (i) - 30, y.y + y.ppp + 50)
      rotate(PI * 0.2)
      text(`${y.minX + i}`, -20, 0)
      pop()
    } else {
      ...
      pop()
      push()
      translate(y.x + y.ppb * 4 * (i) - 20, y.y + y.ppp + 50)
      rotate(PI * 0.5)
      text(`${y.minX + ry + i}`, 0, 0)
      pop()
    }
  }
  // textSize(28)
  ...
  text("Pitch (MPN)", -400, -80);
  pop()
  text("Ontime (Crotchet Beats)", x.x + x.width / 3, x.y + x.ppb + 130);
  text("Ontime (Crotchet Beats)", y.x + y.width / 3, y.y + y.ppb + 130);
