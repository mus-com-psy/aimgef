...
class Map {
                    ...
                    while (j < npts) {
                        ...
                        if (td > tMin && td < tMax && apd >= pMin && apd <= pMax) {
                            ...
                            const he = this.create_hash_entry(
                                [v1[1] - v0[1], td], mode, v0[0], fname, tMin, tMax
                            )
                            this.insert(he)
                            nh++
    ...
    create_hash_entry(vals, mode, onset, filename, tMin, tMax) {
        ...
        switch (mode) {
            ...
            case "triples":
                ...
                if (vals[2] >= vals[3]) {
                    str += "+"
                    str += String((Math.round(10 * vals[2] / vals[3]) / 10).toFixed(1))
                } else {
                    str += "-"
                    str += String((Math.round(10 * vals[3] / vals[2]) / 10).toFixed(1))
                }
                break
            default:
    ...
    histogram(parallelTimes, ctimes, fnams, nosBins = 100, mode = "duples") {
        ...
        return edgeCount.slice(0, 10).map(function (ec) {
