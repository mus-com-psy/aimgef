...
class Encoder(nn.Module):
    def __init__(self,
        ...
        self.track_encoder = nn.LSTM(input_size=input_size,
                                     ...
                                     dropout=dropout,
                                     bidirectional=True)
        self.measure_encoder = nn.LSTM(input_size=hidden_size * 2,
    ...
    def forward(self, x):
        ...
        track_embedding = torch.stack(
            [self.track_encoder(embeddings[i])[0][-1] for i in range(n_tracks)],
            dim=0
        )  # (track, batch, num_directions * hidden_size)
        measure_embedding = self.measure_encoder(track_embedding)[0][-1]  # (batch, num_directions * hidden_size * 2)
