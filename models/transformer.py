...
from models.layers.attention import PositionalEmbedding
from models.layers.attention import Embedding
...
class Transformer(nn.Module):
    def __init__(
        ...
        for _ in range(n_layer):
            self.layers.append(
                AttentionLayer(n_head, d_model, d_head, d_inner, dropout)
            )
    ...
    def decode(self, x, length):
        ...
        for _ in range(length):
            ...
            for layer in self.layers:
                embed = layer(embed, pe, self.w_bias, self.r_bias)
            note = Categorical(
                F.softmax(self.prob(embed)[:, -1], dim=-1)
            ).sample().unsqueeze(dim=1)
